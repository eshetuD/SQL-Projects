--For this Project I have created a Database named as ProjectPortifolio.
--Then, I import the CVS files to this database as tables
	-- The first table named as accident 
	-- And the second table is Vahicle.
--Based up on the data we have in our database, I am going to answer to some questions with SQL to conduct EDA.
------------------------------------------------------------------------------------
...
SELECT 
	[VehicleType], COUNT([AccidentIndex]) AS 'Total Accident', ROUND(AVG(CAST([AgeVehicle] as float)),2) AS 'Average Age'
FROM 
	[dbo].[vehicle]
WHERE 
...
SELECT 
	AgeGroup, COUNT([AccidentIndex]) AS 'Total Accident', ROUND(AVG(CAST([AgeVehicle] as float)),2) AS 'Average Year'
FROM (
	SELECT
...
SELECT 
	[WeatherConditions], COUNT([Severity]) AS 'Total Accident'
FROM 
	[dbo].[accident]
WHERE 
	[Severity] = @Sevierity
...
SELECT 
	[LeftHand],	COUNT([AccidentIndex]) AS 'Total Accident'
FROM 
	[dbo].[vehicle]
GROUP BY 
	[LeftHand]
...
SELECT 
	V.[JourneyPurpose],	COUNT(A.[Severity]) AS 'Total Accident',
	CASE 
...
SELECT 
	A.[LightConditions], V.[PointImpact], AVG(CAST(V.[AgeVehicle] as float)) AS 'Average Vehicle Year'
FROM 
	[dbo].[accident] A
JOIN 
	[dbo].[vehicle] V ON A.[AccidentIndex] = V.[AccidentIndex]
